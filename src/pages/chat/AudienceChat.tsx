    ...
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      <div className={`w-full md:w-80 lg:w-96 border-r border-white/10 flex flex-col bg-background/50 ${selectedUser ? 'hidden md:flex' : 'flex'}`}>
        <div className="h-16 border-b border-white/10 flex items-center px-4 shrink-0 gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate("/dashboard/audience")} className="shrink-0 hover:bg-white/5">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {loadingUsers ? (
            ...
            <div className="text-center py-10 text-muted-foreground">
              ...
              <p className="text-sm mt-1">Discover creators on your dashboard.</p>
            </div>
          ) : (
            chatUsers.map((u) => (
              <button
                    ...
                    <div key={msg.id || idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      ...
                      >
                        {msg.is_priority && !isMe && (
                          <span className="text-[10px] font-bold text-solana-purple mb-1 uppercase tracking-wider">Priority</span>
                        )}
